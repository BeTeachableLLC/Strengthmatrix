...
const App = () => {
  ...
  const workflowWebhookUrl =
    "https://services.leadconnectorhq.com/hooks/MJHmir5Xkxz4EWxcOEj3/webhook-trigger/3da337e1-619a-40b5-8966-bac6ac4edee7";
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // 2) Always generate and upload B&W PDF after successful contact upsert
      setUploadPdfStatus("Generating Black & White PDF...");
      const colorToIdentity = (color) => {
        if (color === "red") return "Decision Makers, Goal Oriented, Result Driven";
        if (color === "yellow") return "Communicators, Participants, Adaptable";
        if (color === "blue") return "Patient, Problem Solver, Good Listener";
        if (color === "green") return "Accurate, Consistent, Analytical";
        return "";
      };

      const bwPdfFile = newpdfBW(
        userDetails,
        formData.Feedback1,
      ...
      setUploadPdfStatus("Uploading Black & White PDF...");
      const body = new FormData();
      body.append("contactId", upsertData.contactId);
      body.append("file", bwPdfFile);
      body.append("fullName", userDetails.fullName || "");
      ...
      setUploadPdfStatus("Submitted and Black & White PDF uploaded successfully.");
      toast.success("Submitted and Black & White PDF uploaded.");
      setShowResults(true);
    } catch (error) {
